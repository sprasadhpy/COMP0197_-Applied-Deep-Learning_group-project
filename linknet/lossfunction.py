...
def supervised_dice_loss(predictions, ground_truth):
    """
    ...
    num_classes = predictions.shape[1]  # Assuming predictions has shape (batch_size, num_classes, height, width)
    ground_truth = F.one_hot(ground_truth, num_classes=3).permute(0, 3, 1, 2).float()

    # Smoothing factor to prevent division by zero
    smooth = 1e-5

    # print('ground_truth_one_hot', ground_truth.shape)
...
def supervised_iou_loss(predictions, ground_truth):
    """
    ...
    iou_loss_library = iou(predictions.cpu(), ground_truth.cpu())
...
def semi_supervised_dice_loss(labeled_predictions, ground_truth, unlabeled_predictions, alpha=0.5):
    """
    ...
    labeled_loss = supervised_dice_loss(labeled_predictions, ground_truth)
    
    # Generate pseudo-labels from unlabeled data
    unlabeled_predictions_pseudo = generate_pseudo_labels(unlabeled_predictions)

    print('unlabeled_predictions_pseudo', unlabeled_predictions_pseudo.shape)
    
    # Compute unlabeled loss
    unlabeled_loss = supervised_dice_loss(unlabeled_predictions, unlabeled_predictions_pseudo)

    # Combine the losses
    return labeled_loss + alpha * unlabeled_loss

def semi_supervised_iou_loss(labeled_predictions, ground_truth, unlabeled_predictions, alpha=0.5):
    """
    ...
    # print('labeled_predictions', labeled_predictions.shape)
    # print('ground_truth', ground_truth.shape)

    # Compute labeled loss
    labeled_loss = supervised_iou_loss(labeled_predictions, ground_truth)
    
    # Generate pseudo-labels from unlabeled data
    unlabeled_predictions_pseudo = generate_pseudo_labels(unlabeled_predictions)
    
    # Compute unlabeled loss
    unlabeled_loss = supervised_iou_loss(unlabeled_predictions, unlabeled_predictions_pseudo)
