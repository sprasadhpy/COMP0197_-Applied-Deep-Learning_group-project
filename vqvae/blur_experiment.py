...
def blurring_experiment(blur_percentage):
    ...
    with torch.no_grad():
        # data_iter = iter(test_dataloader) # Use this and comment out next two lines to test
        # imgs, segs = next(data_iter)
...
print('Blurring experiment started...')
...
with torch.no_grad():
    net = VQVAE(in_channels=3,
                ...
                epsilon=1e-5, )
    net.to(device)
    net.load_state_dict(torch.load("seg_vqvae.pt", map_location=torch.device(device)))

    # BLURRING EXPERIMENT
    # blur_percentage = 100.0  # try 0.0, 25.0, 50.0, 75.0, 100.0
    blur_proportion = [0.0, 25.0, 50.0, 75.0, 100.0]
    for blur in blur_proportion:
        print('Blurring experiment for', blur, '% started...')
